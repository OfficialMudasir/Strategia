...
using Strategia.Migrations.Seed.Host;
...
namespace Strategia.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : StrategiaAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<StrategiaDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{StrategiaConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{StrategiaConsts.ConnectionStringName}", constring);
        }
