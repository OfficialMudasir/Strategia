...
namespace Strategia.Migrations.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly StrategiaDbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = StrategiaConsts.MultiTenancyEnabled ? null : (int?)1;
        ...
        public DefaultLanguagesCreator(StrategiaDbContext context)
        {
            _context = context;
        }
