...
using Strategia.Authentication.TwoFactor.Google;
...
using Strategia.Web.Models.TokenAuth;
using Strategia.Authorization.Impersonation;
using Strategia.Authorization.Roles;
using Strategia.Configuration;
using Strategia.Identity;
using Strategia.Net.Sms;
using Strategia.Notifications;
using Strategia.Security.Recaptcha;
using Strategia.Web.Authentication.External;
...
namespace Strategia.Web.Controllers
{
    ...
    public class TokenAuthController : StrategiaControllerBase
    {
        ...
        private async Task<string> EncryptQueryParameters(long userId, Tenant tenant, string passwordResetCode)
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours)
                .ToString(StrategiaConsts.DateTimeOffsetFormat));
